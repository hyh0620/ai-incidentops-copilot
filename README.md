...
- 独立开发面向 IT 运维与安全场景的智能工单平台，采用前后端分离架构，支持问题提交、附件上传、工单流转、处理任务、人工复核、分析记录和数据看板。
- 搭建以证据为基础的智能分析流程：从用户描述、运行日志和截图中提取并脱敏关键信息，结合向量检索、BM25 关键词检索、RRF 融合和启发式排序查询知识库；分类结果和处理建议均附上对应证据。
- 实现分析过程追踪、历史结果对比、知识库版本管理、受控附件访问和演示权限隔离；构建测试数据集与自动评估工具，验证检索效果、引用可靠性、分类结果和人工复核规则，并支持数据库迁移、Docker 部署和持续集成。
