...
- 知识库摄取与版本化：支持 Markdown、TXT、PDF 文档摄取，复用 KnowledgeBaseArticle / KnowledgeBaseChunk，并记录 KBIngestionRun、kb_version、provider 和耗时。
- 证据约束分诊：默认 `rule_fallback`，输出 provider、rationale、evidence ids、chunk ids、uncertainty 和人工复核原因。
- 可选 LLM 结构化分析：配置 OpenAI-compatible Provider 后，仅基于已脱敏工单摘要和本次检索 chunk 生成结构化建议；引用校验失败会回退规则分诊。
...
- 只读 MCP 工具层：通过 stdio MCP Server 暴露知识检索、工单分析摘要、KB index 状态和摄取历史查询，复用 Demo Persona 权限边界。
...
flowchart LR
  ...
  MCPC["MCP Client"] --> MCPS["MCP Server (stdio)"]
  MCPS --> API
  API --> DB["SQLite"]
  API --> UP["受控附件存储"]
...
![KB ingestion status](docs/images/kb-ingestion-status.png)

![Ticket analysis trace](docs/images/ticket-analysis-trace.png)
...
支持 Markdown、文本和 PDF 文档摄取。文件经过上传校验、文本提取、文档标准化、分块、metadata 绑定、向量索引与关键词索引构建后进入版本化知识库；每次摄取均记录状态、耗时、provider、模型名、fallback reason 和 KB 版本。
...
系统支持 SentenceTransformer 语义检索与本地 hash fallback。默认配置使用 `local_hash_embedding_fallback`，CI、Docker 和离线测试不会下载模型；显式设置 `EMBEDDING_PROVIDER=sentence_transformers` 并安装 `requirements-optional.txt` 后，系统会尝试加载 `SENTENCE_TRANSFORMER_MODEL` 指定的模型。模型下载、加载或推理失败时会回退到 hash fallback，并在 ingestion run、index manifest、API 响应和页面中记录原因。

检索可运行于 `bm25_only`、`dense_only` 和 `hybrid_rrf` 三种模式。评估报告会记录实际 provider、检索指标和延迟；不会假设 Hybrid 一定优于单路检索。

## 检索评估

项目使用 synthetic regression benchmark 比较不同检索模式，并输出 HitRate@K、MRR、nDCG@K、EvidencePrecision、UnsupportedCitationRate 和平均延迟。结果用于验证本地工作流，不代表真实企业线上效果。
...
| 可选分析 | `openai_compatible` | 显式配置 `ANALYSIS_PROVIDER=openai_compatible`、API Base、模型和 API Key 后启用；输出必须通过 JSON schema 与 chunk citation 校验。 |

## MCP 工具接口

项目通过只读 MCP Server 暴露受控的运维知识检索、工单分析、知识库状态和摄取历史工具。MCP 工具复用现有检索、版本和演示权限边界，不提供写入、自动执行或工单状态修改能力。
...
`MCP_DEMO_USER_ID` 必须显式配置。未配置时工具 fail closed；Requester 只能读取自己有权访问的工单，Admin 才能读取 KB index 状态和摄取历史。这是演示身份机制，不代表生产认证。

可用只读工具：

- `search_incident_knowledge(query, top_k, retrieval_mode)`：复用现有 BM25 / Dense / Hybrid RRF 检索，返回 chunk evidence、provider、模型和 fallback 状态。
- `get_ticket_analysis(ticket_id)`：返回已脱敏的工单摘要、当前分析结论、引用 chunk、trace 摘要和复核原因。
- `get_kb_index_status()`：返回 KB 版本、article/chunk 数、provider、模型、fallback、最近摄取和 rebuild 时间，要求 Admin Demo Persona。
- `list_ingestion_runs(limit)`：返回最近摄取记录，要求 Admin Demo Persona。

stdio MCP Client 可使用如下配置形式：
...
默认系统使用离线规则分诊。配置 OpenAI-compatible Provider 后，系统会基于已脱敏的工单信息和本次检索出的知识库证据生成结构化分析结果。
...
模型输出必须符合结构化 JSON schema：`category`、`severity`、`summary`、`recommended_actions`、`cited_chunk_ids`、`confidence`、`review_reason`。`cited_chunk_ids` 必须引用本次检索候选中的 chunk_id；JSON、字段、置信度或引用校验失败时，系统会自动回退至规则分诊，并将原因记录在 Analysis Run 中。

回退条件包括：`missing_api_key`、`timeout`、`provider_error`、`invalid_json`、`invalid_schema`、`invalid_citation`。回退后工单分析继续完成，provider 记录为 `rule_fallback`，trace 标记降级，并进入人工复核策略。
