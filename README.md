...
**项目定位**：当前默认演示环境是一个受控工作流型 IncidentOps Copilot，而不是自由规划或自动执行的 LLM Agent。系统默认离线运行，通过规则、混合检索、证据引用、风险策略和人工复核辅助运维分诊；LLM 仅作为可选的结构化分析增强。

**训练范围**：项目不训练或微调底层模型。合成基准集仅用于回归测试和检索、引用质量评估，不参与线上工单分析。
...
2. 切块采用边界感知的规则策略，优先按 Markdown 标题、列表、空行和句子边界拆分；不是基于 LLM 的语义切块。仅当单段内容过长时才按长度拆分。
3. 分词器保留中文词、英文词、HTTP 状态码、异常名、ORA/SQLSTATE、ECONNRESET 等诊断 token。
4. 向量召回默认使用本地 hash embedding + FAISS。
5. 关键词召回使用 BM25。
6. RRF 融合后进行去重和启发式重排。
7. 返回知识片段级证据摘录、`dense_score`、`lexical_score`、`fusion_score`、`rerank_score`、`final_score`。
...
系统支持可选的 SentenceTransformer 语义向量检索，以及默认离线的 `local_hash_embedding_fallback`。默认 Docker 与 CI 不下载模型，因此使用 hash embedding + FAISS、BM25 和 RRF 融合完成可重复的本地检索。显式设置 `EMBEDDING_PROVIDER=sentence_transformers` 并安装 `requirements-optional.txt` 后，系统会尝试加载 `SENTENCE_TRANSFORMER_MODEL` 指定的模型。模型下载、加载或推理失败时会回退到 hash fallback，并在摄取运行记录、索引清单、API 响应和页面中记录原因。
...
- OCR 复核原因只会在图片、截图或扫描附件实际尝试 OCR 且识别失败或不可用时触发；纯文本、`.log`、`.txt`、`.json`、`.csv` 等附件不会因为未执行 OCR 而被误标。
- 页面会明确区分“工单状态”和“AI 复核状态”；复核原因在正常业务 UI 中显示中文说明，内部英文 code 仅保留在调试或原始分析记录中。
